#include <iostream>
#include "../include/parser.hpp"
#include "fmt/format.h"

auto main() -> int
{
    std::string_view test_str{""};
    
    auto based64_decoded_str = parser::base64_decode(test_str);
    auto inflate_str = parser::inflate(based64_decoded_str.value());
    auto res = parser::url_decode(inflate_str.value());
    
    fmt::print("{}", res.value());

    return 0;
}
